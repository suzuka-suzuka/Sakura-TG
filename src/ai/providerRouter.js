...
export function prioritizeRouteAttempt(plan, preferredAttempt) {
  ...
  return {
    ...
    attempts: [
      preferredAttempt,
      ...plan.attempts.filter((attempt) => attempt !== preferredAttempt),
    ],
  };
}
...
function redactRouterError(error, credential) {
  ...
  const configuredSecret = resolveConfigValue(credential?.apiKey);
  const secrets = [configuredSecret].filter(
...
export function formatRouteAttemptFailure({
  ...
  const fields = [
    "[AI Router] 调用失败",
    `route=${logValue(routeId)}`,
    `attempt=${attemptNumber}/${totalAttempts}`,
    `target=${logValue(attempt?.target?.id)}`,
    `provider=${logValue(attempt?.provider?.id)}`,
    `credential=${logValue(attempt?.credential?.id)}`,
    `model=${logValue(attempt?.requestConfig?.model)}`,
    `status=${error?.status == null ? "unknown" : String(error.status)}`,
    `action=${nextAttempt ? "回退" : "尝试耗尽"}`,
  ];

  if (nextAttempt) {
    fields.push(
      `nextTarget=${logValue(nextAttempt.target?.id)}`,
      `nextProvider=${logValue(nextAttempt.provider?.id)}`,
      `nextCredential=${logValue(nextAttempt.credential?.id)}`,
      `nextModel=${logValue(nextAttempt.requestConfig?.model)}`,
      `delayMs=${Math.max(0, Number(retryDelayMs) || 0)}`
    );
  }

  fields.push(
    `error=${logValue(redactRouterError(error, attempt?.credential))}`
  );
  return fields.join(" ");
}
...
function weightedOrder(items, cursorKey) {
  ...
  const wheel = items.flatMap((item) =>
    Array.from(
      { length: Math.min(positiveInteger(item.weight), 100) },
      () => item
    )
  );
  const seen = new Set();
...
export function orderScheduledItems(items, strategy, cursorKey) {
  const enabled = (Array.isArray(items) ? items : []).filter(
  ...
  if (strategy === "weighted_round_robin") {
    return weightedOrder(enabled, cursorKey);
  }
...
export function resolveGenerationSettings(route, target) {
  ...
  if (openaiReasoningEffort === "inherit") {
    openaiReasoningEffort =
      commonLevel === "off" ? "none" : commonLevel;
  }
  ...
  if (Number.isInteger(explicitBudget) && explicitBudget >= -1) {
    ...
    const resolvedLevel =
      targetLevel === "inherit" ? commonLevel : targetLevel;
...
function buildRequestConfig(route, target, provider, credential) {
  return {
    ...
    apiKey: resolveConfigValue(credential.apiKey),
...
export function createRouteExecutionPlan(routeId, options = {}) {
  ...
  for (const target of targets) {
    ...
    if (!provider) {
      logger.warn(
        `[AI Router] 路由 ${route.id} 的目标 ${target.id} 引用了不存在的供应商 ${target.provider}`
      );
      continue;
    }
    if (protocol && provider.protocol !== protocol) continue;
  ...
  const interleave = (entries) => {
    const result = [];
    const maxCredentials = Math.max(
      0,
      ...entries.map((entry) => entry.credentials.length)
    );
  ...
  if (
    ...
    route.strategy === "weighted_round_robin"
  ) {
    ...
    for (const priority of [...priorityGroups.keys()].sort(
      (a, b) => b - a
    )) {
...
export function resetRoutingCursors() {
